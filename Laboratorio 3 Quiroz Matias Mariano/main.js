...
const addTable = () => {
  ...
  const selectedInputs = [...inputsDefault,  ...inputsCamion, ...inputsAuto, ...inputsActions];
...
const addInputByDefault = (initialValues) => {
  ...
  if (initialValues !== undefined) {
    selectTypeOfPerson.value = initialValues instanceof Camion ? "camion" : "auto";
...
const addInputByTypePerson = (initialValues) => {
  ...
  const selectedInputs = typeField.value === "auto" ? inputsAuto : inputsCamion;
...
const addPerson = (item) => {
  const personObject =
  "carga" in item && "autonomia" in item
      ? new Camion(
          item.id,
          item.modelo,
          item.anoFabricacion,
